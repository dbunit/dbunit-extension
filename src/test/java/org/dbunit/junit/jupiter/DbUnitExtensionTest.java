...
class DbUnitExtensionTest {
    ...
    @Test
    void testBeforeTestExecution_multipleTesterFieldsInSameClass_throwsIllegalStateException() {
        final IDatabaseTester otherTester = mock(IDatabaseTester.class);
        final HasTwoTesters testInstance = new HasTwoTesters(databaseTester, otherTester);
        when(context.getTestInstance()).thenReturn(Optional.of(testInstance));

        assertThatThrownBy(() -> extension.beforeTestExecution(context))
                .as("Expected exception when a class declares more than one IDatabaseTester field.")
                .isInstanceOf(IllegalStateException.class)
                .hasMessageContaining("Multiple IDatabaseTester fields found");
    }
    ...
    static class HasTwoTesters {
        IDatabaseTester databaseTester;
        IDatabaseTester anotherTester;

        HasTwoTesters(final IDatabaseTester databaseTester, final IDatabaseTester anotherTester) {
            this.databaseTester = databaseTester;
