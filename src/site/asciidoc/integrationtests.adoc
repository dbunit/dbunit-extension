...
John Hurst; Jeff Jensen; Andrew Landsverk
...
link:http://mojo.codehaus.org/surefire-maven-plugin/[Maven surefire-plugin].
...
link:http://mojo.codehaus.org/failsafe-maven-plugin/[Maven failsafe-plugin].
...
More easily,
use the convenience script `start-database-via-profile.ps1` or `start-database-via-profile.sh`,
as applicable,
located in the root directory,
e.g. `start-database-via-profile.ps1 postgresql-16`
...
== Stopping a Database Started for IDE Testing

The `pre-integration-test` goal only starts the database's Docker container. Unlike a full
`./mvnw verify` run, it never reaches the `post-integration-test` phase that would
otherwise stop the container again, so the database keeps running — deliberately, so it
stays available for as long as the IDE needs it — until stopped explicitly.

To stop it, run the Maven docker-maven-plugin's stop goal and specify the same Maven
profile used to start it, e.g. `./mvnw io.fabric8:docker-maven-plugin:stop -Ppostgresql-16`

More easily,
use the convenience script `stop-database-via-profile.ps1` or `stop-database-via-profile.sh`,
as applicable,
located in the root directory,
e.g. `stop-database-via-profile.ps1 postgresql-16`

Running the script with no arguments lists the valid profile names.

Derby, H2, and HSQLDB need no stop step: they run embedded and in-memory rather than as
Docker containers, so nothing is left running once the IDE process exits.
...
* dbunit.profile.unsupportedFeatures - comma-separated list of `TestFeature` names to skip during IT execution; see <<unsupported-features-values>>
...
Override the profile property values by:
...
[[unsupported-features-values]]
=== Unsupported Features Values

The `dbunit.profile.unsupportedFeatures` property accepts a comma-separated list of feature names.
When a feature is listed, integration tests that exercise that feature are skipped for the active database profile.
Feature names are case-sensitive and must match exactly.

The valid feature names, their descriptions, and the databases that do not support each are:

[cols="1,3,2", options="header"]
|===
|Feature |Description |Databases that do not support it

|`BLOB`
|Binary Large Object column type; tests inserting and updating BLOB data.
|DB2, Derby, H2, HSQLDB, MariaDB, MySQL, MSSQL, PostgreSQL

|`CLOB`
|Character Large Object column type; tests inserting and updating CLOB data.
|DB2, Derby, H2, HSQLDB, MariaDB, MySQL, MSSQL, PostgreSQL

|`VARBINARY`
|Variable-length binary column type; tests VARBINARY column presence in table metadata.
|Derby

|`TRANSACTION`
|Transaction support; tests `TransactionOperation` wrapping database operations.
|None

|`SCROLLABLE_RESULTSET`
|Scrollable result set cursors; tests `ScrollableResultSetTable` using `TYPE_SCROLL_INSENSITIVE` result sets.
|HSQLDB, MariaDB, MySQL, MSSQL, PostgreSQL

|`INSERT_IDENTITY`
|MS SQL Server identity column insert (`SET IDENTITY_INSERT`); tests `InsertIdentityOperation`.
|DB2, Derby, H2, HSQLDB, MariaDB, MySQL, Oracle, PostgreSQL

|`TRUNCATE_TABLE`
|`TRUNCATE TABLE` statement; tests `TruncateTableOperation`.
|None

|`SDO_GEOMETRY`
|Oracle Spatial Data Object geometry type; tests `OracleSdoGeometryDataType`.
|DB2, Derby, H2, HSQLDB, MariaDB, MySQL, MSSQL, PostgreSQL
...
|DB2, Derby, MariaDB, MySQL, MSSQL, Oracle
|===
...
Maven's profiles (which set the database configuration) and tests fail.
...
dbunit.profile.ddl=hypersonic.sql
...
All test output is written to log files in the `target` directory. When a test failure
needs deeper investigation, grep the log file for the exception directly rather than
relying solely on the Failsafe report summary — the summary is a good starting point,
but it doesn't always surface the actual root-cause exception, especially for failures
several layers down (e.g. inside a JDBC driver or a Docker-started database container).
