...
declaratively instead of calling `runTest()`. `createSchema()` and the
`DRIVER_CLASS`/`CONNECTION_URL`/`ACCOUNT_PREP`/`ACCOUNT_EXPECTED` constants
stay exactly as in step 4 above; the rest of the class is rewritten below to
use `@DbUnitTest`, `@DbUnitTester`, and the lifecycle annotations in place of
the manual `PrepAndExpectedTestCase` wiring:
...
`databaseTester.setTearDownOperation(DELETE_ALL)` from step 4 becomes
`@DbUnitTearDown(operation = DbUnitOperation.DELETE_ALL)` — declared per
method, or once per class when every method needs the same cleanup. Leaving
it off entirely means no teardown runs at all (`NONE` is the actual
default), not some baseline cleanup "for free" — declare it whenever a test
leaves rows behind. See link:testcases/annotations.html[DbUnit Annotations]
