        ...
        implements IDataTypeFactory, IDbProductRelatable
{
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(DefaultDataTypeFactory.class);
    /**
     * Database product names supported.
     */
    private static final Collection DATABASE_PRODUCTS =
            Arrays.asList(new String[] {"derby"});
    ...
    {
        logger.debug("createDataType(sqlType={}, sqlTypeName={}) - start",
                sqlType, sqlTypeName);
            ...
            String tableName, String columnName) throws DataTypeException
    {
        ...
        {
            // Check if the user has set a tolerance delta for this floating
            // point field
            ToleratedDelta delta = _toleratedDeltaMap
                    .findToleratedDelta(tableName, columnName);
            ...
            if (delta != null)
            {
                ...
                // Use a special data type to implement the tolerance for
                // numbers (floating point things)
                NumberTolerantDataType type = new NumberTolerantDataType(
                        "NUMERIC_WITH_TOLERATED_DELTA", sqlType,
