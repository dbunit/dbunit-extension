...
/**
 * Checks whether a type provides its own body for a method it would otherwise inherit -
 * either an interface default method ({@link #overridesDefaultMethod}), or a method a known
 * base class declares ({@link #declaresBelow}). Shared by every {@code org.dbunit.annotation}/
...
public class DefaultMethodOverrideCheck
{
    ...
    /**
     * Returns whether {@code type}, or any class between it and {@code ceiling} (exclusive),
     * declares the named method - the "does a concrete class redeclare this method below a
     * known base class" question, as distinct from {@link #overridesDefaultMethod}'s "does a
     * type override an interface default". Walks {@code type}'s superclass chain, stopping
     * before {@code ceiling} and before {@link Object}.
     *
     * <p>Unlike {@link #overridesDefaultMethod}, a {@link NoSuchMethodException} for one class
     * in the range is not an error: that class simply does not declare the method, and the
     * walk continues to its superclass.
     *
     * @param type The concrete type to start the walk from.
     * @param ceiling The base class to stop before; a declaration on {@code ceiling} itself
     *            does not count.
     * @param methodName The method's name.
     * @param parameterTypes The method's parameter types.
     * @return True when some class in {@code [type, ceiling)} declares the method.
     */
    public static boolean declaresBelow(final Class<?> type, final Class<?> ceiling,
            final String methodName, final Class<?>... parameterTypes)
    {
        Class<?> current = type;
        while (current != null && current != ceiling && current != Object.class)
        {
            try
            {
