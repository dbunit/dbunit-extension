...
The script runs the core build and then the ITs with each database, sequentially, as listed in the database-profiles.txt file located in the root.
...
- dbUnit Organization:
  ...
  - When adding a changes.xml entry, ensure it has these attributes populated and ask when unknown: dev, type, issue, system="github", and due-to
